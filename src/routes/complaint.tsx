...
import { CheckCircle2, Copy, Home, PlusCircle } from "lucide-react";
...
function ComplaintSuccess({
  ...
  const copyRef = async () => {
    try {
      ...
      toast.error(t("copyRef"));
    }
  };

  return (
    <section className="surface rise-in rounded-2xl p-6 text-center sm:rounded-3xl sm:p-10">
      ...
      <h1 className="mt-5 text-xl font-bold sm:text-2xl">{t("complaintSentTitle")}</h1>
      <p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">{t("complaintSentHint")}</p>

      <div className="mx-auto mt-8 max-w-sm rounded-2xl border border-border/70 bg-secondary/30 p-5">
        <p className="text-xs font-medium text-muted-foreground">{t("refNo")}</p>
        <p className="mt-2 font-display text-2xl font-bold tracking-wide text-primary sm:text-3xl" dir="ltr">
          {refNo}
        </p>
        <Button
          type="button"
          variant="outline"
          size="sm"
          className="mt-4 gap-2 touch-manipulation"
          onClick={copyRef}
        >
          <Copy className="size-4" />
          {copied ? t("refCopied") : t("copyRef")}
        </Button>
      </div>
  ...
  return (
    <div className="flex min-h-[100dvh] flex-col">
      ...
      <main className="mx-auto w-full max-w-3xl flex-1 gap-6 px-3 py-6 sm:gap-8 sm:px-4 sm:py-10">
        {submittedRef ? (
          <ComplaintSuccess refNo={submittedRef} onSubmitAnother={resetForm} />
        ) : (
